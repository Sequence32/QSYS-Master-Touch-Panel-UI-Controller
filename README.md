...
3. **Pull Target Layers:** Connect your core, then open the plugin and click **Pull Live UCI Targets**. This will automatically scan your UCI and populate the dropdown menus with all valid layers.
...
- **v7.2.9.14 (Current):** 
  - Restored true standalone capabilities. The `Room Association` property can now be left completely blank without crashing the execution engine.
  - Hardened the `append_suffix` algorithm to strictly bypass appending `-A` room suffixes when `Number of Walls` is set to `0`.
  - Enforced exact layer exclusivity during the `Shutdown Requested` state to prevent rogue layer latching.
